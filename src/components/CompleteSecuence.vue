<template>
  <v-card>
    ...
    <v-container fluid style="height: 80vh; overflow: auto;">
      <v-row :class="$vuetify.breakpoint.mdAndDown ? 'pa-0' : 'fill-height pa-0'" no-gutters align="center" justify="center">
        <v-col v-show="!endQuestions" cols="12" class="py-5 px-0">
          <h3 :class="$vuetify.breakpoint.mdAndDown ? 'text-h5 font-weight-bold' : 'text-h3 font-weight-bold'">
            Completa la secuencia
          </h3>
        </v-col>
        <v-col v-show="!endQuestions" cols="12" class="py-md-6 px--md-0 mx-auto">
          <v-row no-gutters align="center" justify="end">
            <v-col v-for="(value, index) in arraySecuence" :key="index">
              <v-container :class="$vuetify.breakpoint.mdAndDown ? 'pa-0' : 'fill-height pa-0'" fluid>
                <v-row column class="mx-auto">
                  <v-col cols="10" md="8">
                    <v-row align="start" justify="center">
                      <v-col style="position: relative;">
                        <div
                          :class="$vuetify.breakpoint.mdAndDown ? `boxMobile ${checkValue}` : `box ${checkValue}`"
                          v-if="value === ' '"
                        >
                          <a @click="dialog = true" style="text-decoration: none; color: black;">
                            <template>
                              <v-container :class="$vuetify.breakpoint.mdAndDown ? 'pa-0' : 'fill-height pa-0'" fluid>
                                <v-row align="center" justify="center" no-gutters>
                                  <v-col>
                                    <h2
                                      :class="
                                        $vuetify.breakpoint.mdAndDown ? 'body-1 font-weight-medium' : 'text-h2 font-weight-medium'
                                      "
                                    >
                                      {{ valueSelected }}
                                    </h2>
                        ...
                        <div v-else :class="$vuetify.breakpoint.mdAndDown ? 'boxMobile' : 'box'">
                          <template>
                            <v-container :class="$vuetify.breakpoint.mdAndDown ? 'pa-0' : 'fill-height pa-0'" fluid>
                              <v-row align="center" justify="center" no-gutters>
                                <v-col>
                        ...
                        <v-sheet
                          v-if="value === ' '"
                          v-show="dialog"
                          :width="$vuetify.breakpoint.mdAndDown ? '100%' : '200px'"
                          :class="$vuetify.breakpoint.mdAndDown ? '' : 'floatBox-sheet'"
                        >
                            ...
                            <v-row no-gutters align="center" justify="center">
                              <v-col cols="12">
                                <v-btn-toggle
                                  ...
                                  :key="j"
                                  :class="j < numbers.length - 1 ? 'mr-2' : 'mr-0'"
                        ...
                        <v-row v-if="value === ' '" class="mt-4 ma-0 floatBox">
                          <v-col v-if="checkValue === 'boxSuccess'" class="check">
                          ...
                          <v-col v-else-if="checkValue === 'boxError'" class="check">
                            <h6 class="text-h6 font-weight-bold text-left" style="color: #B60006;">
                  ...
                  <v-col v-show="!$vuetify.breakpoint.mdAndDown || index < arraySecuence.length - 1" cols="2" md="3">
                    <v-row align="center" justify="center">
                      <v-col>
                        <div :class="$vuetify.breakpoint.mdAndDown ? 'boxMobile' : 'box'">
                          <template>
                            <v-container :class="$vuetify.breakpoint.mdAndDown ? 'pa-0' : 'fill-height pa-0'" fluid>
                              <v-row align="center" justify="center" no-gutters>
                                <v-col>
        ...
        <v-col v-show="endQuestions" cols="12">
          <v-row justify="center" align="center">
            <v-col cols="12">
              <v-row justify="center" align="center">
                <v-col cols="4" sm="2" md="1" class="pa-md-8">
                  <v-img width="100%" height="100%" :src="require('@/assets/resourceImg/Check.svg')" contain />
                </v-col>
              </v-row>
            </v-col>
            <v-col cols="12">
            ...
            <v-col cols="12" class="mt-md-10">
              <v-btn class="white--text" color="#B60006" x-large href="/" rounded>
                Continuar
              </v-btn>
            </v-col>
          </v-row>
        ...
        <v-col v-show="!endQuestions" cols="12" class="py-6 px-0">
          <v-btn class="white--text" color="#495259" large @click="checkQuestion()" :disabled="!valueSelected">
            Responder
          </v-btn>
        </v-col>
      </v-row>
    ...
    methods: {
      ...
      setArraySecuenceRandom() {
        ...
        if (this.secuenceNum < 5 && this.numTries < 100) {
          ...
          this.indiceRandomValueArray = _.findIndex(this.arraySecuence, function(date) {
            // Busco el indice donde se encuentra, lo extrae y lo guarda
            return date === randomValueOfArray;
          });
          this.arraySequenceMistery = this.arraySecuence.splice(this.indiceRandomValueArray, 1, " ");
          this.arrayHits = [];
          for (let n = 0; n < 3; n++) {
